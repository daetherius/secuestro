...
class AppController extends Controller {
	...
	function beforeFilter(){
		...
		if(in_array('Cart', $this->components)){
			...
			if(!$cart = $this->Session->read('cart')){
				if(!$cart = $this->Cookie->read('cart')){
					$cart = array(
						'Buyer'=>array('nombre'=>'Invitado','created'=>date('d-m-Y H:i:s')),
						'items'=>array(),
						'guest'=>true
					);
				}
		...
		if(strpos($this->action,'admin_')===false){
			...
			if(is_c('nodes',$this) && Cache::read('node_recent') === false){
				$this->loadModel('Node');
				Cache::write('node_recent',$this->Node->find_(array('contain'=>false,'fields'=>array('slug','nombre'),'conditions'=>array('parent_id'=>NULL)),'list'));
			}

			if(is_c('tips',$this) && Cache::read('category_recent') === false){
				$this->loadModel('Category');
				Cache::write('category_recent',$this->Category->find_(array('contain'=>false,'fields'=>array('slug','nombre')),'list'));
			}

			if(is_c('posts',$this) && Cache::read('post_recent') === false){
				$this->loadModel('Post');
				Cache::write('post_recent',$this->Post->find_(array('contain'=>false,'fields'=>array('slug','nombre'),'order'=>'comment_count DESC, created DESC'),'list'));
			}
		}

		/// POLL

			if(($poll = Cache::read('poll_question_recent')) === false){
				$this->loadModel('Poll');
				Cache::write('poll_recent',$poll = $this->Poll->find_(array('contain'=>array('Question'=>array('Answer'))),'first'));
			}
			...
			if($this->Poll->Visitor->find_(array('contain'=>false,'conditions'=>array('ip'=>$ip,'item_id'=>$pid,'item'=>'Poll')),'count')){
				...
				if($answered = $this->Poll->Visitor->find_(array(
					'contain'=>array('Question'),
					'conditions'=>array('ip'=>$ip,'item'=>'Question','item_id'=>$qids),
					'fields'=>array('id','item_id')
				),'list')){ // Si ya hay preguntas contestadas, omitirlas
					foreach($poll['Question'] as $idx => $quest){
						if(empty($quest['Answer']) || ($answered && in_array($quest['id'],$answered))) // Pregunta sin respuestas, ó contestada
