...
echo
	...
	$form->create($_m[0], array('url'=>$this->here,'class'=>'catalog','type'=>'file'));
		...
		if($parent){
			echo
				...
				$form->input('descripcion',array('value'=>$this->data[$_m[0]]['descripcion'],'label'=>'Texto','div'=>array('class'=>'input textarea ordercontenttext'))),
	...
	if(!$parent){
		echo
			...
			$html->link('Agregar Subsección','javascript:;',array('id'=>'elist_adder','class'=>'adminButton add')),
			$html->div('OrderContainer'),
				...
				$this->element('elist',array(
					'fields'=>array('id','nombre'=>array('hide'=>0,'edit'=>1)),
					'options'=>array(
						'data'=>$orderdata,
						'sort'=>1,
						'adder'=>'elist_adder',
