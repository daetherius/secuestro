...
class PaypalComponent extends Object {
	...
	var $return_url	 = '/productos/aprobado';
	...
	function addItem($id, $fields = array()) {
		...
		$fields = array_merge(array('desc'=>'','amt'=>0,'qty'=>0),$existing,$fields);
	...
	function getItemTotals(){
		...
		if(!$this->items){ $this->error('INVALID_ITEMS');return false; }

		foreach($this->items as $item_id => $item){
			$array['L_PAYMENTREQUEST_0_NAME'.$i] = $item_id;
			$array['L_PAYMENTREQUEST_0_DESC'.$i] = $item['desc'];
			$array['L_PAYMENTREQUEST_0_AMT'.$i] = $item['price'].' '.$currency;
